...
exports.parseGeneral = BBPromise.method(function(chtml){
	...
	var clutteredMeta = {
		...
		image: null, //img tag <img src="">
	};

  var htmlLang = chtml('html').first().attr('lang');
  var ogLang = chtml('meta[property="og:locale"]').first().attr('content');

  // build up text content from all texts inside a tag
  var text = clutteredMeta.title + ' ' + chtml('body').find('*').contents().filter(function() {
    return (this.type === 'text' && this.parent && this.parent.type === 'tag');
  }).filter(function() {
    // trim our leading/trailing strings
    return chtml(this).text().trim().replace(/^[\r\n\t\s]+|[\r\n\t\s]+$/g,'');
  });

  if (htmlLang) {
    clutteredMeta['lang'] = htmlLang;
  } else if (ogLang) {
    clutteredMeta['lang'] = ogLang;
  }

  return new BBPromise(function(resolve, reject) {
    cld.detect(text, {}, function(err, result) {
      if (!err && result && result.reliable && result.hasOwnProperty('languages') && result.languages.length > 0) {
        var languages = result.languages;

        // find highest percent match - sort descending
        languages.sort(function (a, b) {
          return b.percent > a.percent;
        })

        if (languages[0].hasOwnProperty('code')) {
          clutteredMeta['lang'] = languages[0].code;
        }
      }

      var images = chtml('img');

      if (images) {
        if (images.length == 1)
          clutteredMeta.image = images.first().attr('src');
        else if (images.length > 1) {
          clutteredMeta.image = [];
          images.each(function (i, elem) {
            clutteredMeta.image.push(chtml(this).attr('src'));
          });
        }
      }
